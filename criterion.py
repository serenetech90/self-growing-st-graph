...
def online_BP(outputs, target):
    ...
    # norm_ade = torch.norm((outputs[0:len(target)] - target), p=2.0)
    # norm_ade = torch.sqrt(torch.norm((outputs[0:len(target)] - target), p=2) )#
    # norm_ade = torch.abs(torch.norm((outputs[0:len(target)] - target)/target, p=2.0))
    # norm_ade = loss(outputs[0:len(target)], target) #/ out_sh[0]
    # norm_ade = torch.sqrt(torch.pow(torch.sum(outputs - target), 2.0)) #/ (out_sh[0] * out_sh[1])

    if out_sh[0] > 1:
        # norm_fde = torch.norm((outputs[end] - target[end_tg]) , p=2.0)
        norm_fde = outputs[end] - target[end_tg]
        norm_fde = norm_fde.pow(2)
        # norm_fde = torch.abs(torch.norm((outputs[end_tg] - target[end_tg])/target[end_tg]
        #                                 , p = 2.0))
        # norm_fde = torch.sqrt(torch.norm((outputs[end_tg] - target[end_tg])
        #                             , 2.0)) # / (out_sh[0])
    else:
        # norm_fde = torch.norm((outputs[:, end] - target[:, end_tg]), p=2.0)
        ...
        # norm_fde = torch.abs(torch.norm((outputs[:, end_tg] - target[:, end_tg])/target[:, end_tg]
        #                                 ,p = 2.0) )
        # norm_fde = torch.sqrt(torch.norm((outputs[:, end_tg] - target[:, end_tg] ), p=2.0))
...
def get_node_positions(graph_step, step=0, max_len=20,curr_graph_nodes=None, istargets=False):
    ...
    #     nodes = graph_step.getNodes()[step-4:step] #sorted()
    ...
    #                     pos[i].append(v.targets[step-4:step])
    #                 else:
    #                     pos[i].append(v.targets[len(v.targets) - 4:len(v.targets)])
    #     else:
    #         pos = []
    #         for i in range(len(nodes)):
    #             pos.append([])
...
def update_hidden_state_tensor(prev_hidden, prev_cell, cut_idx, max_len=20):
    ...
    if old_len < max_len:
        prev_hidden = torch.index_select(prev_hidden, index=torch.LongTensor(cut_idx[0:old_len]), dim=1)
        prev_cell = torch.index_select(prev_cell, index=torch.LongTensor(cut_idx[0:old_len]), dim=1)
    ...
    if cat_len or old_len != max_len:
        prev_cell = torch.cat((prev_cell, torch.zeros(batch_size, cat_len, prev_hidden.size()[2])), dim=1)
        prev_hidden = torch.cat((prev_hidden, torch.zeros(batch_size, cat_len, prev_hidden.size()[2])), dim=1)
